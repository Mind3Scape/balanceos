-- УРОВЕНЬ КРУГА · Э1 (2026-07-16, v2: pending-заявки НЕ считаются составом —
-- иначе «стучащиеся в дверь» раздували знаменатель и день «в ритме» не наступал).
-- XP круга копят закрытые дни людей:
...
as $$
  ...
  mc as (
    ...
    where tm.team_id in (select id from allowed)
      and coalesce(tm.role, 'member') <> 'pending'
    group by tm.team_id
  ),
  days as (
