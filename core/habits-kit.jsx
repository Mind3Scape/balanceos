...
function HabitInviteShareSheet({ habit, link }) {
  ...
  const _ic = (typeof bosCanonColor === "function") ? bosCanonColor(habit?.color) : habit?.color;
  const _icOn = _ic && _ic !== "#0a0a0a" && ("" + _ic).toLowerCase() !== "#8e8e93";
  return (
    <div style={{ padding: "2px 20px 0", color: "var(--text)" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ width: 64, height: 64, borderRadius: 22, margin: "0 auto 12px", background: _icOn ? _ic + "26" : "var(--surface-3)", display: "grid", placeItems: "center", fontSize: 34 }}>{habit?.emoji || "✨"}</div>
