...
function GoalOrbitMini({ centerEmoji, centerColor, habits = [], people = [], size = 128, dark = false, fade = false, progress = null }) {
  ...
  // их просто обрежет карточка (overflow:hidden). ШАГ колец РАЗВЕДЁН (0.14→0.175) и первое кольцо
  // придвинуто к центру (0.315→0.295), чтобы спутники СОСЕДНИХ колец не наезжали при вращении (David:
  // «кружочки со второй орбиты наезжают на первую»): зазор = ringStep − (r_внутр + r_внешн) стал > 0.
  var ringStep = size * 0.175, r0 = size * 0.295;
