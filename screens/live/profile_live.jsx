...
var BOS_OBS_SYSTEM =
  ...
  "как живой человек, который внимательно посмотрел на его дни. Верни СТРОГО JSON-массив из 2–3 сообщений " +
  "без пояснений:\n" +
  '[{"text":"...","chips":["...","..."]}]\n' +
  "text — 2–4 коротких предложения одним куском. Схема речи: что заметил → на чём это видно (число и окно " +
  "времени) → одно маленькое предложение, что сделать. До 320 знаков.\n" +
  "ВЫДЕЛЕНИЕ: 1–3 самых важных куска оберни в **двойные звёздочки** — это главное в сообщении (название " +
  "привычки, день недели, ключевая цифра или сам совет). Другой разметки не используй: ни списков, ни " +
  "заголовков, ни эмодзи.\n" +
  "chips — 2 коротких ответа человека (до 22 знаков), которыми он продолжит разговор.\n" +
  "КАЖДОЕ сообщение — про РАЗНЫЙ факт из данных ниже (день недели, серия, состояние, связка привычек, сфера, " +
  "неделя против прошлой). Не пересказывай один и тот же факт дважды.\n" +
  "Совет должен быть КОНКРЕТНЫМ и выполнимым в приложении: отметить привычку, поменять её дни или время " +
  "напоминания, добавить привычку в названную сферу, снизить цель, позвать человека в совместную привычку, " +
  "отметить состояние. Называй привычку и день по имени. Ничего другого приложение не умеет — не выдумывай " +
  "таймеры, трекеры сна, дыхательные практики, экспорт.\n" +
  "ЗАПРЕЩЕНО: общие слова без цифры («старайся быть последовательным», «баланс важен»), пересказ статистики " +
  "списком, обещания результата, диагнозы, обращение на «вы». Если данных мало — честно скажи, что рано " +
  "делать выводы, и назови, чего именно не хватает.";
...
function bosObsLocalLive(app, st) {
  ...
  if (st.record) out.push({ text: "До рекорда по **«" + st.record.h.name + "»** осталось " + st.record.left + " " + bosObsDaysRu(st.record.left) + ": сейчас серия " + st.record.cur + ", лучшая была " + st.record.best + ". Отметь сегодня — и станет ближе.",
...
function bosObsCleanLive(x) {
  ...
  var t = ("" + (x.text == null ? "" : x.text)).replace(/[#_`>]/g, " ").replace(/\s+/g, " ").trim().slice(0, 380);
