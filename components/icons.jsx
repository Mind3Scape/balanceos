...
/* ── Иконки из ОТКРЫТОГО набора Phosphor (MIT), сетка 256 ──────────────────────────────
   Зачем отдельный вид рядом с нашими штриховыми: в макетах Figma все значки — SF Symbols,
   вставленные глифами шрифта SF Pro (257 штук). Лицензия Apple разрешает их только в
   приложениях для платформ Apple и ПРЯМО ЗАПРЕЩАЕТ веб; на Android шрифта SF Pro нет вовсе
   и вместо значка был бы пустой прямоугольник. Поэтому берём ближайшие по геометрии из
   открытого набора — того же, из которого собран BOS_GLYPHS, — и НЕ рисуем руками.
   Пути скопированы из @phosphor-icons/core@2.1.1/assets/regular, без правок. */
const PhIcon = ({ size = 24, color = "currentColor", d, ...rest }) => {
  delete rest.filled; delete rest.strokeWidth;
  return <svg width={size} height={size} viewBox="0 0 256 256" fill={color} {...rest}><path d={d} /></svg>;
};

const I = {
  /* Phosphor: «взять себе» у гостя (в макете checkmark.circle.badge.plus), «добавить в список»
     на второй кнопке шапки (list.bullet.badge.plus) и звезда у «Администраторов». */
  PlusCircle: (p) => <PhIcon {...p} d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm48-88a8,8,0,0,1-8,8H136v32a8,8,0,0,1-16,0V136H88a8,8,0,0,1,0-16h32V88a8,8,0,0,1,16,0v32h32A8,8,0,0,1,176,128Z" />,
  ListPlus: (p) => <PhIcon {...p} d="M32,64a8,8,0,0,1,8-8H216a8,8,0,0,1,0,16H40A8,8,0,0,1,32,64Zm8,72H216a8,8,0,0,0,0-16H40a8,8,0,0,0,0,16Zm104,48H40a8,8,0,0,0,0,16H144a8,8,0,0,0,0-16Zm88,0H216V168a8,8,0,0,0-16,0v16H184a8,8,0,0,0,0,16h16v16a8,8,0,0,0,16,0V200h16a8,8,0,0,0,0-16Z" />,
  ...
  Info: (p) => <PhIcon {...p} d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216Zm16-40a8,8,0,0,1-8,8,16,16,0,0,1-16-16V128a8,8,0,0,1,0-16,16,16,0,0,1,16,16v40A8,8,0,0,1,144,176ZM112,84a12,12,0,1,1,12,12A12,12,0,0,1,112,84Z" />,
  ...
  CheckCircle: (p) => <PhIcon {...p} d="M173.66,98.34a8,8,0,0,1,0,11.32l-56,56a8,8,0,0,1-11.32,0l-24-24a8,8,0,0,1,11.32-11.32L112,148.69l50.34-50.35A8,8,0,0,1,173.66,98.34ZM232,128A104,104,0,1,1,128,24,104.11,104.11,0,0,1,232,128Zm-16,0a88,88,0,1,0-88,88A88.1,88.1,0,0,0,216,128Z" />,
  Sliders: (p) => <PhIcon {...p} d="M40,88H73a32,32,0,0,0,62,0h81a8,8,0,0,0,0-16H135a32,32,0,0,0-62,0H40a8,8,0,0,0,0,16Zm64-24A16,16,0,1,1,88,80,16,16,0,0,1,104,64ZM216,168H199a32,32,0,0,0-62,0H40a8,8,0,0,0,0,16h97a32,32,0,0,0,62,0h17a8,8,0,0,0,0-16Zm-48,24a16,16,0,1,1,16-16A16,16,0,0,1,168,192Z" />,
  ...
  Group: ({filled, ...p}) => {
    // Два человечка ОДНОЙ минималистичной формы (голова + дуга плеч), стоят рядом БЕЗ грязного
    // пересечения. Контур: только голова + ОТКРЫТАЯ дуга плеч (без плоской линии снизу).
    // Залитый (актив таб-бара): дальний СЕРЫЙ + ближний в цвете (David v673).
    return filled
      ? <Icon {...p} strokeWidth={0}>
          <g fill="#9c9ca3"><circle cx="16.6" cy="7.7" r="2.6"/><path d="M11.9 13.4c1.25-.9 2.85-1.4 4.7-1.4 3.5 0 6.3 2.35 6.3 5.7 0 .68-.55 1.25-1.24 1.25h-5.9z"/></g>
          <g fill={p.color || "currentColor"}><circle cx="9" cy="8" r="3.5"/><path d="M2.1 20.2c0-3.55 2.95-6.1 6.9-6.1s6.9 2.55 6.9 6.1c0 .74-.6 1.35-1.34 1.35H3.44C2.7 21.55 2.1 20.94 2.1 20.2z"/></g>
        </Icon>
      : <Icon {...p}>
          <circle cx="16.4" cy="7.7" r="2.4"/><path d="M15.4 12.4c.5-.2 1.05-.3 1.6-.3 2.95 0 5.3 2.15 5.3 5.1" fill="none"/>
          <circle cx="9" cy="8" r="3.4"/><path d="M2.5 20c0-3.5 2.9-5.9 6.5-5.9s6.5 2.4 6.5 5.9" fill="none"/>
        </Icon>;
  },
  ...
  Heart: ({filled, ...p}) => <Icon {...p}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" fill={filled ? (p.color || "currentColor") : "none"}/></Icon>,
  ...
  Sneaker: ({filled, ...p}) => filled ? <Icon {...p} strokeWidth={0}><g fill={p.color || "currentColor"}>
      <path d="M3.1 15.7c0-1.7.8-2.9 2.15-3.5l1.05-.45V8.7c0-.62.5-1.12 1.12-1.12.48 0 .9.3 1.06.75l.52 1.4 4.75 2.2c.5.23.9.62 1.12 1.13l.5 1.15c.3.68.95 1.15 1.7 1.24l2.05.25c1.6.2 2.68 1.02 2.68 2.32v.15H3.1z"/>
      <path d="M2.5 16.35h19c.55 0 .95.4.95.97v.4c0 1.02-.82 1.75-1.98 1.75H3.53c-1.16 0-1.98-.73-1.98-1.75v-.4c0-.57.4-.97.95-.97z"/>
      <rect x="8.8" y="9.5" width="1.25" height="3.1" rx="0.62" transform="rotate(-24 9.4 11)"/>
      <rect x="11.15" y="10.45" width="1.25" height="3.1" rx="0.62" transform="rotate(-24 11.8 12)"/>
    </g></Icon> : <Icon {...p}><path d="M3.1 15.4V9c0-.6.5-1 1.1-1 .5 0 .9.3 1 .8l.5 1.6 5 2.3c.5.2.9.6 1.1 1.1l.5 1.1c.3.7 1 1.1 1.7 1.2l2 .3c1.4.2 2.4 1 2.4 2.2v.4H3.1z"/><path d="M2.6 17h18.8"/></Icon>,
};
