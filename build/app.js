...
function PhoneApp() {
  ...
  var onDragEnd = e => {
    ...
    var remaining = pop ? d.w - d.dx : d.dx;
    var speed = Math.abs(d.vx || 0) * 1000; // px/s
    var dur = Math.max(0.12, Math.min(0.34, speed > 4 ? remaining / speed : 0.3));
    setDrag({
      dx: pop ? d.w : 0,
      w: d.w,
      releasing: true,
  ...
  var dragTrans = drag && drag.releasing ? "transform " + (drag.dur || 0.3) + "s var(--ios-ease), opacity " + (drag.dur || 0.3) + "s var(--ios-ease)" : "none";
