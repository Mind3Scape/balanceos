...
function loadDemoBundle() {
  ...
  var inject = src => new Promise((res, rej) => {
    var s = document.createElement("script");
    s.src = src;
    s.onload = res;
    s.onerror = () => rej(new Error("demo bundle failed: " + src));
    document.head.appendChild(s);
  });
  // Sequential, in the same order the old static <script> tags used (home → habits → community).
  _demoBundleP = ["home", "habits", "community"].reduce((chain, n) => chain.then(() => inject("build/screens/demo/" + n + ".js?v=" + v)), Promise.resolve()).catch(e => {
    _demoBundleP = null;
    try {
      console.warn(e);
    } catch (_) {}
...
function BottomSheet({
  ...
  useEffect(() => {
    if (!render) return;
    try {
      document.body.classList.add("bos-sheet-open");
    } catch (e) {}
    return () => {
      try {
        document.body.classList.remove("bos-sheet-open");
      } catch (e) {}
    };
  }, [render]);
...
function bosReferralId() {
  try {
    ...
    if (sp) {
      ...
      if (/^hb_/.test(sp)) {
        var pp = sp.indexOf("__");
        if (pp >= 0) return sp.slice(pp + 2);
      }
    }
  } catch (e) {}
  try {
...
function bosJoinSharedHabitId() {
  try {
    ...
    if (sp && /^hb_/.test(sp)) {
      var rest = sp.slice(3);
      var pp = rest.indexOf("__");
      return pp >= 0 ? rest.slice(0, pp) : rest;
    }
  } catch (e) {}
  try {
    var q = new URLSearchParams(window.location.search).get("habit");
    if (q) return q;
  } catch (e) {}
  return null;
}
...
function bosShare(url, text) {
  var tg = null;
  try {
    tg = window.__TG || window.Telegram && window.Telegram.WebApp || null;
  } catch (e) {}
  try {
...
function BosAvatar({
  ...
  if (isEmoji) return /*#__PURE__*/React.createElement("div", {
    style: Object.assign(base, {
      ...
      lineHeight: 1,
      textShadow: bare ? "0 1px 4px rgba(0,0,0,0.3)" : "none"
    })
  }, ("" + avatar).slice(6));
  return /*#__PURE__*/React.createElement("div", {
    style: base
...
function BosOrbFace({
  ...
  var t0 = tint && tint[0] || "#ffd97a",
    t2 = tint && tint[2] || "#d97757";
  var base = Object.assign({
    width: size,
    height: size,
    borderRadius: "50%",
  ...
  if (isMemoji) return /*#__PURE__*/React.createElement("div", {
    style: Object.assign(base, {
      background: "url(./assets/people/" + avatar + ".png) center/cover no-repeat",
  ...
  return /*#__PURE__*/React.createElement("div", {
    style: Object.assign(base, {
      background: "url(./assets/sphere.png) center/cover no-repeat, radial-gradient(circle at 30% 30%, " + t0 + ", " + t2 + ")",
      boxShadow: "inset -3px -5px 12px rgba(0,0,0,0.22)"
    })
  });
}
function AppProvider({
  ...
  var [themeOverride, setThemeOverride] = useState(() => {
    try {
      return localStorage.getItem("bos:theme") || "auto";
    } catch (e) {
      return "auto";
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem("bos:theme", themeOverride);
    } catch (e) {}
  }, [themeOverride]);
  ...
  var toggleHabit = id => setHabits(hs => hs.map(h => {
    ...
    try {
      if (on && h.reminder && h.reminder.on && _liveCloud() && window.bosCloud.markReminderDone) window.bosCloud.markReminderDone(h.cloudId || h.id);
    } catch (e) {}
  ...
  var removeHabit = id => {
    setHabits(hs => {
      ...
      try {
        if (h && _liveCloud() && window.bosCloud.deleteReminder) window.bosCloud.deleteReminder(h.cloudId || h.id);
      } catch (e) {}
  ...
  var _remSyncedRef = useRef(false);
  var _tzSyncedRef = useRef(false);
  useEffect(() => {
    if (mode !== "live" || !_liveCloud()) return;
    var tz = -new Date().getTimezoneOffset(); // Москва UTC+3 → +180
    if (!_tzSyncedRef.current && window.bosCloud.saveTz) {
      _tzSyncedRef.current = true;
      try {
        window.bosCloud.saveTz(tz);
      } catch (e) {}
    }
    if (!_remSyncedRef.current && window.bosCloud.upsertReminder && habits && habits.length) {
      ...
      habits.filter(h => h && h.reminder && h.reminder.on && !h.shelved).forEach(h => {
        try {
          window.bosCloud.upsertReminder(h.cloudId || h.id, {
            name: h.name,
            emoji: h.emoji,
            time: h.reminder.time || "09:00",
            days: Array.isArray(h.days) ? h.days : null,
            tzOffset: tz,
            active: true
          });
        } catch (e) {}
  ...
  var [claimedChallenges, setClaimedChallenges] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("bos:claimedXP") || "{}") || {};
    } catch (e) {
      return {};
    }
  });
  useEffect(() => {
    ...
    (habits || []).forEach(function (h) {
      var c = h && h.challenge;
      if (!c || !c.key || claimedChallenges[c.key]) return;
      var need = c.days | 0;
      if (need > 0 && bosStreak(h.log) >= need) {
        add = add || {};
        add[c.key] = c.bonus | 0;
      }
    });
    (goals || []).forEach(function (g) {
      var c = g && g.challenge;
      if (!c || !c.key || claimedChallenges[c.key]) return;
      if (g.target > 0 && (g.current || 0) >= g.target) {
        add = add || {};
        add[c.key] = c.bonus | 0;
      }
    });
    (teams || []).forEach(function (t) {
      var c = t && t.challenge;
      if (!c || !c.key || claimedChallenges[c.key]) return;
      if (t.target > 0 && (t.current || 0) >= t.target) {
        add = add || {};
        add[c.key] = c.bonus | 0;
      }
    });
    if (add) {
      var merged = Object.assign({}, claimedChallenges, add);
      setClaimedChallenges(merged);
      try {
        localStorage.setItem("bos:claimedXP", JSON.stringify(merged));
      } catch (e) {}
      if (window.tgHaptic) {
        try {
  ...
  var [spentXP, setSpentXP] = useState(() => {
    try {
      return parseInt(localStorage.getItem("bos:spentXP") || "0", 10) || 0;
    } catch (e) {
      return 0;
    }
  });
  var spendXP = (amount, ref, meta) => {
    ...
    setSpentXP(function (prev) {
      var next = (prev | 0) + a;
      try {
        localStorage.setItem("bos:spentXP", String(next));
      } catch (e) {}
      return next;
    });
  ...
  var lastPubRef = useRef(null); // подпись последней опубликованной «орбиты» (Вселенная) — дедуп записей
  ...
  useEffect(() => {
    ...
    var _li = typeof bosLevelInfoLive === "function" ? bosLevelInfoLive(_xp) : {
      level: 1,
      pct: 2
    };
    var pubHabits = (habits || []).map(function (h) {
      return {
        e: h.emoji,
        c: h.color
      };
    ...
    var t = setTimeout(function () {
      try {
        window.bosCloud.savePublicStats({
          ...
          habits: pubHabits,
          goals: (goals || []).length,
          people: invitedCount | 0
        });
        lastPubRef.current = sig;
      } catch (e) {}
    }, 600);
    return function () {
      clearTimeout(t);
  ...
  var _briefSignal = mode === "live" ? [typeof bosTodayKey === "function" ? bosTodayKey() : "", (habits || []).filter(h => h && h.done).length,
  // habits checked today
  (habits || []).length,
  // added / removed a habit
  ...
  useEffect(() => {
    ...
    var on = true;
    var today = _briefSignal.split("|")[0];
    var sameDay = !!(cached && cached.signal && cached.signal.split("|")[0] === today);
      ...
      if (window.bosCloud && window.bosCloud.enabled()) {
        ...
        window.bosCloud.signIn(_refBy).then(function (u) {
          ...
          window.bosCloud.loadSnapshot().then(function (snap) {
            ...
            if (Object.keys(_mClaimed).length !== Object.keys(claimedChallenges || {}).length) {
              setClaimedChallenges(_mClaimed);
              try {
                localStorage.setItem("bos:claimedXP", JSON.stringify(_mClaimed));
              } catch (e) {}
            }
            var _mSpent = Math.max(snap && snap.data && snap.data.spentXP | 0 || 0, spentXP | 0);
            if (_mSpent !== (spentXP | 0)) {
              setSpentXP(_mSpent);
              try {
            ...
            if (snap && snap.data && cloudAt >= localAt) {
              ...
              try {
                ...
                var _mi = _tk ? _mMoods[_tk] : undefined;
              ...
              if (Object.keys(_mMoods).length > Object.keys(_cloudMoods).length || Object.keys(_mNotes).length > Object.keys(_cloudNotes).length) {
                window.bosCloud.saveSnapshot({
                  teams: Array.isArray(d.teams) ? d.teams : saved && saved.teams || [],
                  dayMoods: _mMoods,
                  dayNotes: _mNotes,
                ...
                return window.bosCloud.joinSharedHabit(_joinShareId).then(function (sh) {
                  ...
                  setHabits(function (prev) {
                    if ((prev || []).some(function (x) {
                      return x.shareCode === _joinShareId;
                    })) return prev || [];
                    var nh = bosRollHabit({
                      id: _nid(),
                      cloudId: _uuid(),
                    ...
                    try {
                      window.bosCloud.upsertHabit(nh);
                    } catch (e) {}
  ...
  useEffect(() => {
    ...
    try {
      var c = parseInt(localStorage.getItem("bos:cache:inv:" + persistId) || "", 10);
      if (isFinite(c) && c > 0) setInvitedCount(c);
    } catch (e) {}
    window.bosCloud.invitedPeople().then(function (list) {
      if (on && Array.isArray(list)) {
        setInvitedCount(list.length);
        try {
          localStorage.setItem("bos:cache:inv:" + persistId, "" + list.length);
        } catch (e) {}
      }
    }).catch(function () {});
    return function () {
  ...
  var refreshTeamGoalXP = function () {
    ...
    try {
      var c = parseInt(localStorage.getItem("bos:cache:tgxp:" + persistId) || "", 10);
      if (isFinite(c) && c > 0) setTeamGoalXP(c);
    } catch (e) {}
    window.bosCloud.myTeamGoalXP().then(function (xp) {
      setTeamGoalXP(xp || 0);
      try {
        localStorage.setItem("bos:cache:tgxp:" + persistId, "" + (xp || 0));
      } catch (e) {}
    }).catch(function () {});
  };
  useEffect(function () {
