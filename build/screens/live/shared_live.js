...
function bosGoalHero(color, isDark) {
  ...
  if (!accent) {
    return {
      bg: isDark ? "radial-gradient(135% 100% at 50% -12%, rgba(255,255,255,0.06), rgba(255,255,255,0) 60%), linear-gradient(157deg, #26262e 0%, #191920 100%)" : "radial-gradient(135% 100% at 50% -12%, rgba(255,255,255,0.6), rgba(255,255,255,0) 60%), linear-gradient(157deg, #F2F2F6 0%, #E4E4EC 100%)",
      ink: isDark ? "#fff" : "#16161a",
      sub: isDark ? "rgba(255,255,255,0.72)" : "rgba(22,22,26,0.56)",
      btnBg: isDark ? "rgba(255,255,255,0.10)" : "rgba(255,255,255,0.75)",
      btnInk: isDark ? "#fff" : "#1b1b1f",
      chipBg: isDark ? "rgba(255,255,255,0.10)" : "rgba(255,255,255,0.7)",
      chipInk: isDark ? "rgba(255,255,255,0.92)" : "#26262c",
      chipAiBg: isDark ? "rgba(255,255,255,0.14)" : "rgba(255,255,255,0.95)",
      chipAiInk: isDark ? "#cdd8f0" : "#3a5a8c",
      onDark: isDark
    };
  }
  if (isDark) {
    var d1 = bosMixHex(accent, "#0d0f14", 0.16),
      d2 = bosMixHex(accent, "#0d0f14", 0.30);
    return {
      ...
      chipInk: "rgba(255,255,255,0.95)",
      chipAiBg: "rgba(255,255,255,0.92)",
      chipAiInk: "#3a5a8c",
      onDark: true
    };
  }
...
async function bosNotifCollectLive(app) {
  ...
  ...teams.map(async t => {
    try {
      ...
      var _lrRaw = localStorage.getItem("bos:chatread:" + t.cloudId);
      var lastRead = _lrRaw ? new Date(_lrRaw).getTime() : 0;
...
function bosTeamUnreadCacheGet(cloudId) {
  var c = _bosTeamUnreadCache[cloudId];
  return c && Date.now() - c.at < 60000 ? c : null;
}
async function bosTeamUnreadPeek(cloudId) {
  ...
  try {
    ...
    return _bosTeamUnreadCache[cloudId] || null;
  }
}
...
function bosTeamUnreadClear(cloudId) {
  if (cloudId) _bosTeamUnreadCache[cloudId] = {
    at: Date.now(),
    count: 0
  };
}
try {
  window.addEventListener("bos:notifSeenChanged", function () {
    _bosTeamUnreadCache = {};
  });
} catch (e) {}
...
function bosSphereFor(item) {
  ...
  for (var i = 0; i < BOS_SPHERES.length; i++) {
    // 1) по названию (корни слов)
    var id = BOS_SPHERES[i].id,
      kws = BOS_SPHERE_KW[id] || [];
    for (var j = 0; j < kws.length; j++) {
      if (name.indexOf(kws[j]) >= 0) return id;
    }
  }
  var emo = "" + (item.emoji || ""); // 2) по эмодзи
  if (emo) {
    for (var k = 0; k < BOS_SPHERES.length; k++) {
      var id2 = BOS_SPHERES[k].id;
      if ((BOS_SPHERE_EMO[id2] || "").indexOf(emo) >= 0) return id2;
    }
  }
...
function bosHabitStrength(h) {
  ...
  var marks = 0;
  try {
    marks = h.log ? Object.keys(h.log).length : 0;
  } catch (e) {}
...
function bosWheelData(app) {
  ...
  return {
    spheres: spheres,
    overall: Math.round(total / spheres.length * 100),
    filled: filled
  };
}
function bosZoneColor(v) {
...
function BosBalanceWheelLive(props) {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    r: R * 0.7,
    fill: "none",
    stroke: dark ? "rgba(52,199,89,0.4)" : "rgba(52,199,89,0.34)",
    strokeWidth: "1",
...
function StateSheetLive(props) {
  var app = typeof useApp === "function" ? useApp() : null;
  var sheet = typeof useSheet === "function" ? useSheet() : {
    close: function () {}
  };
  var isDark = !!(app && app.themeOverride === "dark");
  var tk = typeof bosTodayKey === "function" ? bosTodayKey() : new Date().toISOString().slice(0, 10);
  var faces = typeof MOOD_FACES !== "undefined" ? MOOD_FACES : ["😣", "😞", "😕", "😐", "🙂", "😄", "🤩"];
  var words = typeof MOOD_WORDS !== "undefined" ? MOOD_WORDS : ["Тяжело", "Плохо", "Так себе", "Нормально", "Неплохо", "Хорошо", "Отлично"];
  var initV = React.useMemo(() => {
    var di = app && app.dayMoods && app.dayMoods[tk];
    if (di != null) {
      var n = BOS_STATE.length - 1;
      return n ? Math.max(0, Math.min(1, di / n)) : 0.6;
    }
    return 0.72;
  }, []);
  ...
  var trackRef = React.useRef(null),
    dragRef = React.useRef(false),
    lastB = React.useRef(-1);
  ...
  var breath = 1 + Math.sin(t * 0.8) * 0.03;
  var bucket = bosStateStepFromV(val);
  var tint = bosStateTintForV(val);
  var face = faces[bucket] || "🙂",
    word = words[bucket] || "Ровно";
  var PAD = 13;
  ...
  var cardText = isDark ? "#fff" : "var(--text)";
  var subMuted = isDark ? "rgba(255,255,255,0.55)" : "var(--text-4)";
  var fieldBg = isDark ? "rgba(255,255,255,0.06)" : "var(--surface-3)";
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, props && props.evening ? "Вечерняя отметка" : "Как ты сейчас"), /*#__PURE__*/React.createElement("div", {
    style: {
      position: "relative",
      width: 128,
      height: 128,
    ...
    style: {
      ...
      inset: -18,
      borderRadius: "50%",
      background: "radial-gradient(circle, " + tint[1] + "44 0%, " + tint[1] + "14 45%, transparent 72%)",
      filter: "blur(14px)"
    }
  }), /*#__PURE__*/React.createElement("div", {
    style: {
    ...
    onPointerMove: e => {
      if (dragRef.current) setFromX(e.clientX);
    },
    onPointerUp: () => {
    ...
    style: {
      ...
      transition: dragRef.current ? "none" : "left 0.1s ease"
    }
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
  ...
  }, /*#__PURE__*/React.createElement("span", null, "\u0442\u044F\u0436\u0435\u043B\u043E"), /*#__PURE__*/React.createElement("span", null, "\u043E\u0442\u043B\u0438\u0447\u043D\u043E")), /*#__PURE__*/React.createElement("input", {
    value: note,
    onChange: e => setNote(e.target.value),
    placeholder: "\u041F\u0430\u0440\u0430 \u0441\u043B\u043E\u0432, \u0435\u0441\u043B\u0438 \u0445\u043E\u0447\u0435\u0448\u044C\u2026",
    style: {
      width: "100%",
      marginTop: 16,
      background: fieldBg,
...
function StateInviteLive({
  ...
  var openState = function () {
    if (sheet && sheet.open) sheet.open(/*#__PURE__*/React.createElement(StateSheetLive, null));else if (navigate) navigate("mood");
  };
  return /*#__PURE__*/React.createElement("button", {
    onClick: openState,
    className: "tap",
...
function HomeGalleryContentLive({
  ...
  }, "\u041F\u0440\u0438\u0432\u044B\u0447\u043A\u0438 \u0438 \u0446\u0435\u043B\u0438 \u043F\u043E\u044F\u0432\u043B\u044F\u044E\u0442\u0441\u044F \u043D\u0430 \u0433\u043B\u0430\u0432\u043D\u043E\u0439 \u0441\u0430\u043C\u0438. \u0421\u043F\u0440\u044F\u0442\u0430\u0442\u044C \u2014 \u043C\u0438\u043D\u0443\u0441\u043E\u043C \u0432 \u0440\u0435\u0436\u0438\u043C\u0435 \u0442\u0440\u044F\u0441\u043A\u0438, \u0432\u0435\u0440\u043D\u0443\u0442\u044C \u2014 \u0432 \xAB\u0410\u0440\u0445\u0438\u0432\u0435\xBB."));
}
...
function TeamTileLive({
  ...
  var [tileUnread, setTileUnread] = React.useState(() => {
    var c = typeof bosTeamUnreadCacheGet === "function" ? bosTeamUnreadCacheGet(_ck) : null;
    return c ? c.count : 0;
  });
  React.useEffect(() => {
    if (!_ck || ctx.mode || typeof bosTeamUnreadPeek !== "function") return;
    var on = true;
    bosTeamUnreadPeek(_ck).then(r => {
      if (on && r) setTileUnread(r.count || 0);
    }).catch(() => {});
    var f = () => {
      var c = typeof bosTeamUnreadCacheGet === "function" ? bosTeamUnreadCacheGet(_ck) : null;
      setTileUnread(c ? c.count : 0);
    };
    window.addEventListener("bos:notifSeenChanged", f);
    return () => {
      on = false;
      window.removeEventListener("bos:notifSeenChanged", f);
    };
  }, [_ck]);
  ...
  var _inFlow = orbitPeople.filter(p => p.active).length;
  var _pulseTxt = pct >= 1 ? "🎉 Готово" : _inFlow > 0 ? "🔥 " + _inFlow + " сегодня" : members.length ? "👥 " + members.length : null;
  var _tileFloat = !ctx.mode && orbit && (_pulseTxt || tileUnread > 0) ? /*#__PURE__*/React.createElement(React.Fragment, null, _pulseTxt && /*#__PURE__*/React.createElement("span", {
    style: {
      position: "absolute",
      top: 10,
      left: 11,
      ...
      whiteSpace: "nowrap",
      textOverflow: "ellipsis",
      background: isDark ? "rgba(0,0,0,0.24)" : "rgba(255,255,255,0.66)",
  ...
  }, _pulseTxt), tileUnread > 0 && /*#__PURE__*/React.createElement("span", {
    style: {
      position: "absolute",
      top: 9,
      right: 10,
  ...
  }, "\uD83D\uDCAC ", tileUnread > 99 ? "99+" : tileUnread)) : null;
  ...
  }, _tileFloat, orbit ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    "aria-hidden": true,
    style: {
      position: "absolute",
      left: "50%",
...
function GoalTileLive({
  ...
  if (g.deadline) {
    try {
      var _ms = new Date(g.deadline).getTime() - Date.now();
      if (!isNaN(_ms)) _dLeft = Math.ceil(_ms / 86400000);
    } catch (e) {}
  }
  var _linkedT = gp.linked && gp.linked.length ? gp.linked : [];
  var _goalChipTxt = pct >= 1 ? "🎉 Готово" : _dLeft === 0 ? "⏳ сегодня" : _dLeft != null && _dLeft > 0 && _dLeft <= 90 ? "⏳ " + _dLeft + " дн" : _linkedT.length ? "✓ " + _linkedT.filter(h => h.done).length + "/" + _linkedT.length + " сегодня" : null;
  var _goalFloat = !ctx.mode && orbit && _goalChipTxt ? /*#__PURE__*/React.createElement("span", {
    style: {
      position: "absolute",
      top: 10,
      left: 11,
  ...
  }, _goalFloat, orbit ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    "aria-hidden": true,
    style: {
      position: "absolute",
      left: "50%",
...
function CardStyleMenuLive({
  ...
  var app = typeof useApp === "function" ? useApp() : null;
  var darkOn = !!(app && app.themeOverride === "dark");
  var setDark = v => {
    if (app && app.setThemeOverride) app.setThemeOverride(v ? "dark" : "light");
  };
  React.useEffect(() => {
    if (!open) return;
  ...
  var formBtn = (key, icon, cur, onPick) => /*#__PURE__*/React.createElement("button", {
    key: key,
    onClick: () => onPick(key),
    className: "tap",
    style: {
  ...
  }, formBtn("rect", RC, hs.form, k => setH({
    form: k
  })), formBtn("square", SQ, hs.form, k => setH({
    form: k
  ...
  }, toggleRow("Люди", hs.faces, v => setH({
    faces: v
  ...
  }, formBtn("banner", BN, gs.form, k => setG({
    form: k
  })), formBtn("square", SQ, gs.form, k => setG({
    form: k
  }))), divider, /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 0
  ...
  }, "\u041E\u0440\u0431\u0438\u0442\u044B \u043F\u043E\u043A\u0430\u0437\u044B\u0432\u0430\u044E\u0442 \u043F\u0440\u0438\u0432\u044B\u0447\u043A\u0438 \u0438 \u043B\u044E\u0434\u0435\u0439 \u0432\u043E\u043A\u0440\u0443\u0433 \u0446\u0435\u043B\u0438 \u2014 \u043F\u0440\u0435\u0432\u044C\u044E, \u0432\u043E\u043A\u0440\u0443\u0433 \u0447\u0435\u0433\u043E \u043E\u043D\u0430.")) : /*#__PURE__*/React.createElement(React.Fragment, null, toggleRow("Тёмная тема", darkOn, setDark), toggleRow("Эффект стекла", glassOn, setGlass), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11.5,
      color: "rgba(10,10,10,0.42)",
      lineHeight: 1.4,
  ...
  }, "\u0422\u0451\u043C\u043D\u0430\u044F \u0442\u0435\u043C\u0430 \u2014 \u0432\u0435\u0441\u044C \u044D\u043A\u0440\u0430\u043D \u0432 \u0442\u0451\u043C\u043D\u044B\u0445 \u0442\u043E\u043D\u0430\u0445. \u0421\u0442\u0435\u043A\u043B\u043E \u2014 \u0433\u043B\u044F\u043D\u0446\u0435\u0432\u044B\u0435 \u0431\u043B\u0438\u043A\u0438; \u0432\u044B\u043A\u043B\u044E\u0447\u0438 \u0434\u043B\u044F \u043F\u043B\u043E\u0441\u043A\u043E\u0433\u043E \u0432\u0438\u0434\u0430."), typeof onArchiveList === "function" && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      onClose();
      onArchiveList();
    },
...
function MoodWidgetLive({
  ...
  var _sheet = typeof useSheet === "function" ? useSheet() : null;
  var _openState = () => {
    if (_sheet && _sheet.open) _sheet.open(/*#__PURE__*/React.createElement(StateSheetLive, null));else if (navigate) navigate("mood");
  };
...
function HomeHeroSwipeLive({
  ...
  var aiBrief = totalCount && doneCount >= totalCount ? "День закрыт — ты в потоке. Так держи ритм." : AI_BRIEF[mood && mood.t] || "Один маленький шаг сегодня — и ритм на твоей стороне.";
...
function PartnersMapLive({
  ...
  var [failed, setFailed] = React.useState(false); // Яндекс не смог (нет ключа/офлайн/домен) → резерв
  ...
  React.useEffect(function () {
    ...
    bosLoadYandexMaps().then(function (ymaps) {
      ...
      try {
        ...
        if (alive) setFailed(true); /* карта не построилась → стилизованный резерв */
      }
    }).catch(function () {
      if (alive) setFailed(true); /* нет ключа/офлайн → стилизованный резерв */
    });
    return function () {
      alive = false;
      try {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, failed && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("svg", {
    viewBox: "0 0 366 232",
  ...
  }))), !ready && !failed && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "absolute",
      inset: 0,
      display: "grid",
    ...
    style: {
      ...
      height: 26,
      borderRadius: "50%",
      border: "2.5px solid " + (isDark ? "rgba(255,255,255,0.18)" : "rgba(0,0,0,0.12)"),
      borderTopColor: isDark ? "#dfe7dd" : "#2b3a2b"
    }
  })), /*#__PURE__*/React.createElement("div", {
    ref: mapRef,
    "aria-hidden": !ready,
